...
class GRPOConfig:
    ...
    # v6 state augmentation: if non-empty, prepend raw IQA(x_0) broadcast
    # channels to the policy input so the policy can condition actions on
    # *current quality*. Each name must be a key in `iqa_heads` (e.g. 'clipiqa',
    # 'musiq'). Values are normalised to roughly [0, 1] before broadcasting:
    # clipiqa is already in [0, 1]; musiq is divided by 100.
    state_aug_iqa: List[str] = field(default_factory=list)
...
class GRPOTrainer:
    ...
    def _compute_state_aug(self, x0: Tensor) -> Optional[Tensor]:
        """Compute [B, K, H, W] IQA-on-x_0 broadcast channels, or None."""
        names = self.cfg.state_aug_iqa
        if not names:
            return None
        chans = []
        with torch.no_grad():
            for n in names:
                if n not in self._iqa_heads:
                    continue
                s = self._iqa_heads[n](x0, x0)       # [B] raw IQA on x_0
                if s.ndim != 1:
                    s = s.view(-1)
                if n == "musiq":
                    s = s / 100.0                     # ~[0, 1]
    ...
    def _collect_group(self, x0: Tensor, aug: Optional[Tensor]) -> List[Trajectory]:
    ...
    def _terminal_borda_advantage(
        ...
        # Trajectory final_state is 3-channel; if state is augmented, states[t]
        # has extra channels but final_state was stored as 3-channel (image only).
        with torch.no_grad():
            for name, fn in self._iqa_heads.items():
                # Unified (x_0, x_K) signature: unary heads ignore x_0.
                # Baseline via fn(x_0, x_0). For input-anchored heads
                # (identity_l1, lpips), this is 0 by construction.
                s0 = fn(x0, x0)
                if s0.ndim != 1:
                    s0 = s0.view(-1)
                sk_list, score_list = [], []
                for traj in trajs:
                    sk = fn(x0, traj.final_state)
                    if sk.ndim != 1:
                        sk = sk.view(-1)
                    sk_list.append(sk)
                    if targets is not None and name in targets:
                        ...
                        # Fallback: Δ-from-baseline if no target. For input-
                        # anchored heads s0=0 so this equals the raw neg-
                        # distance (identity_l1: -|x_K-x_0|, etc.).
    ...
    def update(self, x0: Tensor) -> Dict[str, float]:
        ...
        cfg = self.cfg
        device = x0.device
        aug = self._compute_state_aug(x0)
        trajs = self._collect_group(x0, aug)
